...
#if 0
/*
	功能：	放入队列。如果队列满，会挤掉队列头。
	返回：	队列状态异常，返回-1;
			正常入队，返回0.
			挤掉了头数据，1.
	注意：	如果队列满，会挤掉队列头。
		这部分功能不完善，后续应该改为：如果队列满，则阻塞等待，或者返回false.
*/
template <class T>
...
int MyQueue<T>::push(const T *pData)
{
	...
	if(curDepth == queueMaxDepth)
	{
		//std::cout << "push end. curDepth == queueMaxDepth." << std::endl;
		lock.clear(std::memory_order_release);	// 解锁
		return -2;
	}
